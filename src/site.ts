...
function loadEventSessions(id: String) {
    let eventData: Sessionize.Event

    const getTemplate = (templateId: string) => ((document.getElementById(templateId) as HTMLTemplateElement)
    ...
    const getSpeakerInfo = (sessionSpeakers: Array<string>) => eventData.speakers.filter((s) => sessionSpeakers.indexOf(s.id) >= 0)
    ...
    const addPopupHandler = () => {
        ...
        document.querySelectorAll('.agenda-session').forEach((element) => {
            const div = element as HTMLDivElement
            const dataSlotId = element.attributes['data-slot-id'].value
